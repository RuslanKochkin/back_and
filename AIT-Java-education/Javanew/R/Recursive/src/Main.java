public class Main {
    public static void main(String[] args) {
        ...
        printNumbers(10);
        System.out.println(stringPol("potop"));
        System.out.println(stringPol("pottop"));
        System.out.println(stringPol("poeettop"));
    ...
    //Задание 2
    //Дано слово, состоящее только из строчных латинских букв. Проверьте, является ли это слово палиндромом. Выведите yes, если является и no - если не является.
    //
    //P.S. Палиндром - это слово, которое одинаково читается слева направо, например: око, потоп.
    public static String stringPol(String str) {
        if (str.length() <= 1) {
            return "yes";
        } else {
            ...
            return stringPol(str.substring(1, str.length() - 1));
        }
    }
}